...
namespace Data_Structure_Exp
{
    class Exp_main
    {
        ...
        {
            Exp1.SeqList<int> seq = new Exp1.SeqList<int>(100);
            List<int> tl = new List<int>(100);
            int[] array_t = new int[] { 28, 16, 32, 12, 60,2,5,72};
            foreach (var i in array_t)
            {
                seq.Append(i);
            }
            Exp7.QuickSort2(ref seq,0,seq.GetLength());
        }
    }
